...
const currentTitle = computed(() => {
  const exactMatch = allMenu.value.find(m => route.path === m.path)
  if (exactMatch) return exactMatch.name
  if (route.path.startsWith('/knowledge/edit')) return '知识库管理'
  const prefixMatch = allMenu.value.find(m => route.path.startsWith(m.path) && m.path !== '/')
